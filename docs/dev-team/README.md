...
| 2026-06-05 | [`requirements-github-enterprise.md`](requirements-github-enterprise.md) | Draft — blocked on OQ-GHE-1 (public exposure) and OQ-GHE-2 (network reachability) | Delta: GHES support — host-parameterized API base, raw URL, repoUrl, and dedicated PAT; two headline risks must be resolved before Architect engagement |
| 2026-06-05 | [`adr-006-ghes-host-parameterization.md`](adr-006-ghes-host-parameterization.md) | Proposed | GHES host-parameterization: per-entry `host` in `repos.json` drives all URLs+token; raw content via Contents API (resolves OQ-GHE-3); github.com special-case kept (90+12 tests stay green); per-host PAT routing `GHES_SCAN_PAT`→`GHES_TOKEN` (OQ-GHE-4); no schema change; OQ-GHE-1/2 deferred as blocking-for-rollout constraints |
